...
app.controller('stockCtrl', ['$scope', '$state', '$stateParams', '$modal', '$log', 'Stock', function ($scope, $state, $stateParams, $modal, $log, Stock) {
   ...
   // searchStock_start();
    ...
    function searchStock_start() {
        // alert($scope.searchText + 'scope');
        Stock.search(0)
            .then(function (data) {
                var _barcode;
                $scope.stocks = Stock.stocks;
                _barcode = $scope.stocks[0].barcode
                var _idd
                _idd = $scope.stocks[0].idd;
                alert(_idd);
                $scope.stockDetail(idd, _barcode);
                // $scope.SubResizeScreen();
            });
    }
    ...
    $scope.open = function (size) {        
        var modalInstance = $modal.open({
            animation: false,
            backdrop: 'static',
            templateUrl: 'app/stock/AddEditStock.cshtml',
